...
layout(binding = 5) uniform sampler2D gAlbedo;           // <- albedo (glTF baseColor)
...
// NUEVO: interruptor para mostrar solo textura
uniform bool gTexOnly = false;
...
float CalcShadowFactorPointLight(vec3 LightToPixel)
{
    ...
    float bias = 0.015;
    if (SampledDistance + bias < Distance) return 0.25;
    else return 1.0;
}
...
float CalcShadowFactorBasic(vec3 LightDirection, vec3 Normal)
{
    ...
    if (Depth + bias < ShadowCoords.z) return 0.05;
    else return 1.0;
}

float CalcShadowFactorPCF(vec3 LightDirection, vec3 Normal)
{
    if (gShadowMapWidth == 0 || gShadowMapHeight == 0) return 1.0;
    vec3 ProjCoords = LightSpacePos0.xyz / LightSpacePos0.w;
    vec3 ShadowCoords = ProjCoords * 0.5 + vec3(0.5);
    float DiffuseFactor = dot(Normal, -LightDirection);
    float bias = mix(0.001, 0.0, DiffuseFactor);
    ...
    for (int y = -HalfFilterSize ; y < -HalfFilterSize + gShadowMapFilterSize ; y++) {
        for (int x = -HalfFilterSize ; x < -HalfFilterSize + gShadowMapFilterSize ; x++) {
            ...
            if (Depth + bias < ShadowCoords.z) ShadowSum += 0.0;
            else ShadowSum += 1.0;
       }
   }
...
float CalcShadowFactorWithRandomSampling(vec3 LightDirection, vec3 Normal)
{
    ...
    for (int i = 0 ; i < 4 ; i++) {
        ...
        if (Depth + bias < ShadowCoords.z) Sum += 0.0; else Sum += 1.0;
...
float CalcLayeredFogFactor()
{
    ...
    if (gCameraWorldPos.y > gLayeredFogTop) {
...
// -------------------- main con modo “solo textura” --------------------
void main()
{
    // ?? DEBUG: pinta rojo para comprobar que el modelo carga
    if (gTexOnly) {
